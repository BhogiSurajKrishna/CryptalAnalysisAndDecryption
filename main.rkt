...
(define (crack-cipher strategies key) ;; Returns list of encryption keys.
  ...
  ;; DISPLAY A KEY AS SOON AS YOU FIND IT USING (show-key key)
   ...
   (define l-keys (map (lambda (x) (utils:add-substitution x key)) strategies))
  (define l-dictclo (map (lambda (x) (algo:dictionary-closure x)) l-keys))
  (define (answer liskeys)
    (if (null? liskeys) '()
    (cond [(equal? (algo:secret-word-enumeration (car liskeys)) #f) (answer (cdr liskeys))]
           [else  (utils:show-key (algo:secret-word-enumeration (car liskeys)))
                  (append (list (algo:secret-word-enumeration (car liskeys))) (answer (cdr liskeys))) ])))
                                                                (answer l-dictclo))
